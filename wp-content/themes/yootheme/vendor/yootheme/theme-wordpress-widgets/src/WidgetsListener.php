...
class WidgetsListener
{
    ...
    public function isActiveSidebar(Config $config, $active, $sidebar)
    {
        return $active ||
            ...
            in_array($sidebar, [
                $config('~theme.header.search'),
                $config('~theme.header.social'),
            ]) ||
    ...
    public function afterSidebar(Config $config, View $view, $sidebar)
    {
        ...
        if (
            ($sidebar == $social || ($social && $sidebar == 'mobile')) &&
    ...
    {
        if ($instance === false || ($this->sidebar === null && empty($args['yoo_element']))) {
            return $instance;
        }
