...
class SchemaPrinter
{
    ...
    private static function printFilteredSchema(
        ...
        $directives = array_filter($schema->getDirectives(), function ($directive) use (
            $directiveFilter
        ) {
        ...
        return implode(
            ...
            array_filter(
                array_merge(
                    ...
                    array_map(function ($directive) use ($options) {
    ...
    private static function printObject(ObjectType $type, array $options)
    {
        ...
        $directives = self::printDirectives($type, $options);
        ...
        return self::printDescription($options, $type) .
            "type {$type->name}$implementedInterfaces$directives {\n" .
            ...
            array_map(
                function ($f, $i) use ($options) {
                    return self::printDescription($options, $f, '  ', !$i) .
                        ...
                        ': ' .
                        (string) $f->getType() .
                        self::printDirectives($f, $options) .
    ...
    private static function printInputValue($arg)
    {
        ...
        if ($arg->defaultValueExists()) {
            $argDecl .=
                ' = ' . Printer::doPrint(AST::astFromValue($arg->defaultValue, $arg->getType()));
        }
        return $argDecl;
    }
    ...
    private static function printDirectives($fieldOrType, $options)
    {
        $directives = [];
    ...
    private static function printDescription(
        ...
        $hasLeadingSpace =
            isset($lines[0]) &&
            (substr($lines[0], 0, 1) === ' ' || substr($lines[0], 0, 1) === '\t');
        if (!$hasLeadingSpace) {
            $description .= "\n";
        }
