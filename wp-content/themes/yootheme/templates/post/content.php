...
$image = function ($attr) use ($config, $view) {
    ...
    if ($view->isImage($src) == 'svg') {
        $thumbnail = $image->replace($view->image($src, ['width' => $config('~theme.post.image_width'), 'height' => $config('~theme.post.image_height'), 'uk-img' => true, 'alt' => $alt]));
    } else {
        $thumbnail = $image->replace($view->image([$src, 'thumbnail' => [$config('~theme.post.image_width'), $config('~theme.post.image_height')], 'srcset' => true], ['uk-img' => true, 'alt' => $alt]));
    }
